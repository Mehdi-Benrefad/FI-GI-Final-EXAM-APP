...
class MapViewController: UIViewController , MKMapViewDelegate , CLLocationManagerDelegate{
    ...
    func tracePositionInTheMap (){
        ...
        //Ajout des Markers [quand on presse lontement su une position de l'ecran une nouvelle annotation s'ajoutes]
         let mylongpress = UILongPressGestureRecognizer(target: self, action: #selector(MapViewController.longPress(gestureRecognizer:)))
    ...
    @IBAction func onSwipe(_ sender: UISwipeGestureRecognizer) {
        if sender.state == .ended {
                   print("hello")
        self.performSegue(withIdentifier: "mapToSideMenu", sender: nil)
        }
    }
